...
description: 处理飞书知识库(Feishu Wiki/Docx)相关操作的技能。当用户需要：读取/写入飞书文档、搜索/查询知识库内容或节点、创建/更新/删除飞书知识库空间或节点、同步本地文件到飞书、获取飞书文档内容、或者任何涉及"飞书"、"Feishu"、"知识库"数据交互的任务时，必须使用此技能。即使任务看起来像简单的文本处理或搜索问答，只要最终目标或数据来源是飞书，就立即触发此技能。
---
