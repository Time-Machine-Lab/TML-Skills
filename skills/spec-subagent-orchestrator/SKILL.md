...
Default stance after execution is ready: the master should prefer dispatch over direct execution unless there is a concrete delegation blocker.
...
4. Route to the execution mode.
   ...
   Use `subagent-driven-development` when tasks are mostly independent and can be executed as separate atomic slices in the current session.
   Use `subagent-supervisor-constitution` when the work is medium/high risk, cross-module, drift-prone, or expensive to review incorrectly.
...
3. Can the next work be written as one or more atomic executor tasks?
   ...
   If yes, delegate unless there is a specific distortion risk from delegation.
...
- Do not use "the master already understands the area" as a positive routing signal for direct execution.
...
- If a step can be reduced to one owner, one deliverable, and one acceptance check, prefer dispatch over master execution.
...
6. Local-execution exception
   If not delegating, state the concrete blocker that prevents safe delegation right now.
   Format it as `delegation blocker: <short phrase>`.
