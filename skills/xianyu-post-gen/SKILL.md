...
- 存储位置：`references/prohibited_elements.md`
- 用途：记录闲鱼平台或用户指定的违禁词、敏感词。生成内容时必须严格规避这些词汇。
...
- 必须读取并严格避开 `references/prohibited_elements.md` 中定义的违禁元素（如“会员制”、“GPT”等）。
...
在生成内容后（进入封面图流程前），agent 必须自动执行以下审核步骤：

### 6.1 表情合规性审核
- 读取 `output/post.md`。
- 读取 `assets/emojis.json`。
- 检查规则：帖子中出现的 `[...]` 格式表情，必须在 `emojis.json` 的 **Values** 中存在。
- 修正动作：若发现非法表情（如 `[烟花]`），必须将其替换为 `emojis.json` 中含义相近的合法表情（如 `[火]`），或直接删除。

### 6.2 违禁词审核
- 读取 `references/prohibited_elements.md`。
- 检查规则：帖子内容不得包含任何违禁词。
- 修正动作：若发现违禁词，必须进行同义替换或删除，确保合规。

**注意**：Agent 必须在通过所有审核后，才能询问用户是否需要封面图。
...
## 8. 图片生成（通过 image-generation skill）
